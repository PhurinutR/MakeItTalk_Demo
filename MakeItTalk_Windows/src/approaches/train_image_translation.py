...
class Image_translation_block():
    ...
    def single_test(self, jpg=None, fls=None, filename=None, prefix='', grey_only=False):
        ...
        # os.system('del output/frames/*')
        if not os.path.exists('output/frames'):
            os.system(f'md output/frames')
        else:
            os.system('del output/frames/*')
        
        if not os.path.exists('output/landmark_frames'):
            os.system(f'md output/landmark_frames')
        else:
            os.system('del output/landmark_frames/*')
        
        # os.system(f'md output/frames/{filename[9:-16]}')
        ...
        for i, frame in enumerate(fls):
            ...
            for j in range(g_out.shape[0]):
                ...
                cv2.imwrite(f"output/frames/frame{i+1}.png", frame)
                cv2.imwrite(f"output/landmark_frames/FL{i+1}.png", img_fl)
                # cv2.imwrite(f"output/frames/{filename[9:-16]}/frame{i+1}.png", frame)
                # cv2.imwrite(f"output/frames/{filename[9:-16]}/FL{i+1}.png", img_fl)
                writer.write(frame.astype(np.uint8))

        writer.release()
        # time_elapsed = time.time() - st
        # print(f'Time - only video: {time_elapsed // 60:.0f}m {time_elapsed % 60:.0f}s')

        if(filename is None):
            filename = 'v'
        os.system('ffmpeg -loglevel error -y -i out.mp4 -i examples/tmp.wav -pix_fmt yuv420p -strict -2 output/result.mp4')
        # os.system('ffmpeg -loglevel error -y -i out.mp4 -i {} -pix_fmt yuv420p -strict -2 output/{}.mp4'.format(
        #     'examples/commentaries/'+filename[9:-16]+'.wav', filename[9:-16]))
        os.system('del out.mp4')
        if(os.path.isfile('examples/tmp.wav')):
            os.remove('examples/tmp.wav')

        time_elapsed = time.time() - st
        print(f'Time - ffmpeg add audio for {filename[9:-16]}: {time_elapsed // 60:.0f}m {time_elapsed % 60:.5f}s\n')
