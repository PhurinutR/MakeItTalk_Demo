...
class Audio2landmark_content(nn.Module):
    ...
    def __init__(self, opt_parser, num_window_frames=18, in_size=80, lstm_size=AUDIO_FEAT_SIZE, use_prior_net=False, hidden_size=256, num_layers=3, drop_out=0, bidirectional=False):
    ...
    def forward(self, au, face_id):
        ...
        #In this code, output2 refers to delta_q_t in the document; it is the value telling the change due to the speech content.
        if self.opt_parser.No_Content_Animate:
            output2.zero_()
            print("\nSpeech Content Animation has been removed this time!")
            print("This is the delta_q_t size of {} right now (the change due to the speech content)".format(output2.size()))
            print(output2)
...
class Audio2landmark_speaker_aware(nn.Module):
    ...
    def __init__(self, opt_parser, audio_feat_size=80, c_enc_hidden_size=256, num_layers=3, drop_out=0,
    ...
    def forward(self, au, emb, face_id, fls, z, add_z_spk=False, another_emb=None):
        ...
        #In this code, fl_pred refers to delta_p_t in the document; it is the value telling the change due to longer time-dependencies.
        if self.opt_parser.No_Speaker_Aware_Animate:
            fl_pred.zero_()
            print("\nSpeaker Aware Animation has been removed this time!")
            print("This is the delta_p_t size of {} right now (the change due to longer time-dependencies)".format(fl_pred.size()))
            print(fl_pred)
