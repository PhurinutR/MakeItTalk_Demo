...
class Image_translation_block():
    ...
    def single_test(self, jpg=None, fls=None, filename=None, prefix='', grey_only=False):
        ...
        # os.system('rm output/frames/*')
        if not os.path.exists('output/frames'):
            os.system(f'mkdir output/frames')
        else:
            os.system('rm output/frames/*')
        
        if not os.path.exists('output/landmark_frames'):
            os.system(f'mkdir output/landmark_frames')
        else:
            os.system('rm output/landmark_frames/*')
        
        # os.system(f'mkdir output/frames/{filename[9:-16]}')
