...
class Audio2landmark_model():
    ...
    def __init__(self, opt_parser, jpg_shape=None):
        ...
        # Step 3: Load model
        self.G = Audio2landmark_speaker_aware(drop_out=0.5,
    ...
    def __train_pass__(self, au_emb=None, centerize_face=False, no_y_rotation=False, vis_fls=False):
        ...
        for i, batch in enumerate(dataloader):
            ...
            #show an example of Content Embedding
            print("Below is an example of Content Embedding shape of {} from one of {} frames (After windowing)".format(inputs_au[0].size(),inputs_au.size(dim=0)))
            print(inputs_au[0])
            ...
            for key in keys: # ['45hn7-LXDX8']: #['sxCbrYjBsGA']:#
                ...
                if(vis_fls):
                    ...
                    Vis(fls=fake_fls_np, filename=video_name.split('\\')[-1].split('/')[-1], fps=62.5,
                        audio_filenam=os.path.join('examples', 'tmp.wav'))
                    # Vis(fls=fake_fls_np, filename=video_name.split('\\')[-1].split('/')[-1], fps=62.5,
                    #     audio_filenam=os.path.join('examples/commentaries', video_name.split('\\')[-1].split('/')[-1]+'.wav'))
